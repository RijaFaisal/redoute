...
- There's a second, structural problem underneath the first: an LLM-based auditor that reads source code to find vulnerabilities is itself reading untrusted input. Code under audit can contain text crafted to look like instructions to the model, making the auditor a prompt-injection surface by construction, not by accident.
...
Exit codes are consistent across both commands: `0` nothing high/critical (or, for `patch`, every high/critical finding got an accepted patch), `1` a high/critical finding remains, whether unfixed, unpatched, or its patch was rejected, `2` couldn't run at all (missing file, unreachable model).
